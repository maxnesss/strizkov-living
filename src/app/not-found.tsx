...
export default function NotFound() {
  return (
    ...
    >
      <section className="mx-auto max-w-[1240px] overflow-hidden rounded-[36px] border border-[#b89a7c33] bg-white/70 px-6 pb-10 pt-10 shadow-[0_40px_120px_-84px_rgba(98,69,45,0.28)] backdrop-blur-[18px] sm:px-8 lg:px-[34px] lg:pb-12 lg:pt-12">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <Link
              key={item.href}
              className="rounded-[24px] border border-[#b89a7c2e] bg-white/76 px-5 py-5 transition hover:bg-white"
              href={item.href}
            >
              <p className="text-[22px] font-semibold text-[#3f3125]">{item.label}</p>
              <p className="mt-2 text-sm leading-7 text-[#7d6a59]">{item.text}</p>
            </Link>
          ))}
        </div>
      </section>
